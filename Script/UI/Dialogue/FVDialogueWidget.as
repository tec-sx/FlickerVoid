...
class UFVDialogueWidget : UUserWidget
{
    ...
    UFVDialogueSubsystem DialogueSub;
    ...
    void Construct()
    {
        ...
        DialogueSub = Cast<UFVDialogueSubsystem>(Subsystem::GetGameInstanceSubsystem(UFVDialogueSubsystem));
        ...
        DialogueSub.OnDialogueLineStarted.AddUFunction(this, n"HandleLineStarted");
        DialogueSub.OnDialogueResponsesAvailable.AddUFunction(this, n"HandleResponsesAvailable");
    ...
    void Destruct()
    {
        if (DialogueSub != nullptr)
        {
            DialogueSub.OnDialogueLineStarted.Unbind(this, n"HandleLineStarted");
            DialogueSub.OnDialogueResponsesAvailable.Unbind(this, n"HandleResponsesAvailable");
    ...
    UFUNCTION()
    void HandleLineStarted(const FFVDialogueLine&in Line, const FFVDialogueContext&in Context)
    {
        SetVisibility(ESlateVisibility::HitTestInvisible);
        ResponsesContainer.ClearChildren();

        FFVSpeakerInfo SpeakerInfo;
        FText SpeakerName = DialogueSub.GetSpeakerInfo(Line.SpeakerId, SpeakerInfo)
    ...
    UFUNCTION()
    private void HandleResponsesAvailable(const TArray<FFVDialogueResponse>&in Responses)
    {
        if (SkipHintText != nullptr)
        {
        ...
        for (int32 i = 0; i < Responses.Num(); ++i)
        {
            ...
            if (Entry != nullptr)
            {
                ...
                Entry.Initialize(DialogueSub, Responses[i], i);
    ...
    {
        if (DialogueSub != nullptr && DialogueSub.IsInConversation())
        {
            DialogueSub.SkipCurrentLine();
        }
    }
}
...
class UFVResponseEntryWidget : UUserWidget
{
    ...
    void Initialize(UFVDialogueSubsystem Sub, FFVDialogueResponse Response, int32 Index)
    {
        BoundSub    = Sub;
        StoredIndex = Index;
