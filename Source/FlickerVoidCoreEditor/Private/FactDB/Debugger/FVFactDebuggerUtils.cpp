...
namespace Utils
{
	...
	ETagMatchType MatchFavorites(FGameplayTag CheckedTag)
	{
	...
	void CopyItem(const FFVFactTreeItemPtr& SourceItem, TArray<FFVFactTreeItemPtr>& OutDestArray)
	{
		FFVFactTreeItemPtr& NewItem = OutDestArray.Add_GetRef(MakeShared<FFVFactTreeItem>());
		*NewItem = *SourceItem;
		NewItem->InitItem();
	}

	void FilterFavoriteFactItemChildren(const TArray<FFVFactTreeItemPtr>& SourceArray,
	                                    TArray<FFVFactTreeItemPtr>& OutDestArray, const FFVFilterOptions& Options);
	void FilterMainFactItemChildren(const TArray<FFVFactTreeItemPtr>& SourceArray, TArray<FFVFactTreeItemPtr>& OutDestArray,
	                                const FFVFilterOptions& Options);

	void CopyItemIfMainChildrenMatch(const FFVFactTreeItemPtr& SourceItem, TArray<FFVFactTreeItemPtr>& OutDestArray,
	                                 const FFVFilterOptions& Options)
	{
		TArray<FFVFactTreeItemPtr> FilteredChildren;
	...
	void CopyItemIfFavoritesChildrenMatch(const FFVFactTreeItemPtr& SourceItem, TArray<FFVFactTreeItemPtr>& OutDestArray,
	                                      const FFVFilterOptions& Options)
	{
		TArray<FFVFactTreeItemPtr> FilteredChildren;
	                                    ...
	                                    TArray<FFVFactTreeItemPtr>& OutDestArray, const FFVFilterOptions& Options)
	{
		...
		{
			return MatchSearchBox(Options.SearchBarStrings, TagString) && MatchSearchToggle(
				Options.SearchToggleStrings, TagString);
		};

		for (const FFVFactTreeItemPtr& SourceItem : SourceArray)
		{
			...
			case ETagMatchType::Child:
				// straight to filtering children, even if this item matched - it is not favorite by itself
				CopyItemIfFavoritesChildrenMatch(SourceItem, OutDestArray, Options);
				continue;
	                                ...
	                                const FFVFilterOptions& Options)
	{
		...
		for (const FFVFactTreeItemPtr& SourceItem : SourceArray)
		{
			...
			{
				if (SourceItem->Children.Num() > 0)
				// even if this item has value - children can be without value and therefore shoundn't be visible
				{
			...
			case ETagMatchType::Parent:
				// parent tag is favorite, continue only if we are showing favorites in main tree
			...
			case ETagMatchType::Child:
				// some child tag is favorite, if we are showing favorites in main tree - continue, otherwise - straight to filtering children
				if (Options.bShowFavoritesInMainTree)
				{
					bNeedsFilteringByText = true;
					break;
