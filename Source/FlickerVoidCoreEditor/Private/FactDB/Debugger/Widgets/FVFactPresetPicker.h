...
class SFVFactPresetPicker : public SCompoundWidget
{
public:
	DECLARE_DELEGATE_OneParam(FOnSelectionChanged, const UFVFactPreset*)

	SLATE_BEGIN_ARGS(SFVFactPresetPicker)
		{
		}
...
private:
	...
	// List view
	TSharedRef<ITableRow> HandleGeneratePresetWidget(TSharedPtr<FAssetData> AssetData,
	                                                 const TSharedRef<STableViewBase>& OwnerTable);
	void HandleSelectionChanged(TSharedPtr<FAssetData> AssetData, ESelectInfo::Type Type);
	void HandleSortListView(EColumnSortPriority::Type SortPriority, const FName& ColumnName,
