...
DECLARE_DELEGATE_OneParam(FFVAssetManagerStartupJobSubstepProgress, float /*NewProgress*/);

/** Handles reporting progress from streamable handles */
struct FFVAssetManagerStartupJob
{
	FFVAssetManagerStartupJobSubstepProgress SubstepProgressDelegate;
	TFunction<void(const FFVAssetManagerStartupJob&, TSharedPtr<FStreamableHandle>&)> JobFunc;
	...
	FFVAssetManagerStartupJob(const FString& InJobName, const TFunction<void(const FFVAssetManagerStartupJob&, TSharedPtr<FStreamableHandle>&)>& InJobFunc, float InJobWeight)
		: JobFunc(InJobFunc)
		, JobName(InJobName)
